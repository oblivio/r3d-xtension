...
| **Phishing site cloner** | One-click clone of any authenticated page. The proxy fetches the real page with captured auth context, rewrites all asset URLs, and injects SPA-aware credential capture hooks — form submit interception, password field blur listeners, fetch/XHR body intercept, MutationObserver for dynamically added forms, and post-interaction cookie/storage snapshots. Victim-facing URL at `/p/{id}/`. Captured credentials beacon to the dashboard in real-time. |
